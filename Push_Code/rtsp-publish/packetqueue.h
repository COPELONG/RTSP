...
public:
    ...
    int pushPrivate(AVPacket *pkt, MediaType media_type)
    {
        ...
        if(E_AUDIO_TYPE == media_type) {
            ...
            if(queue_.empty()) {
                ...
                std::cout<<"again set audio PTS"<<std::endl;
        ...
        if(E_VIDEO_TYPE == media_type) {
            ...
            if(queue_.empty()) {
                ...
                std::cout<<"again set video PTS"<<std::endl;
    ...
    int PopWithTimeout(AVPacket **pkt, MediaType &media_type, int timeout)
    {
      ...
      // std::cout<<"empty "<<std::endl;
        *pkt        = mypkt->pkt;
        ...
        free(mypkt);
            ...
            MyAVPacket *mypkt1 = queue_.front(); //读取队列首部元素，这里还没有真正出队列
            *pkt        = mypkt->pkt;
            media_type  = mypkt->media_type;
      std::cout<<"lulong "<<std::endl;
            if(E_AUDIO_TYPE == media_type) {
                ...
                stats_.audio_size -= mypkt->pkt->size;
                // 持续时长怎么统计，不是用pkt->duration
                audio_front_pts_ = mypkt1->pkt->pts;
            }
            if(E_VIDEO_TYPE == media_type) {
                stats_.video_nb_packets--;      // 包数量
                stats_.video_size -= mypkt->pkt->size;
                // 持续时长怎么统计，不是用pkt->duration
                video_front_pts_ = mypkt1->pkt->pts;
            }
        }
    ...
    void GetStats(PacketQueueStats *stats)
    {
        ...
        if(queue_.empty()){
            std::cout<<"audio_duration = 0;"<<std::endl;
        audio_duration = 0;
        }
        ...
        if(queue_.empty()){
             std::cout<<"video_duration = 0;"<<std::endl;
        video_duration = 0;
        }
